...
{{ dynamic }}
        <div class="content-top">
          ...
          <h2>{{block page_name}}{{ if $gimme->template->name == "front.tpl" }}<a href="{{ url options="root_level" }}">Das Online-Magazin für Luzern und Zug</a>{{ elseif $gimme->template->name == "search_index.tpl" }}Suchergebnisse{{ elseif $gimme->template->name == "ticker.tpl" }}Ticker{{ elseif $gimme->template->name == "404.tpl" }}Server Error 404{{ elseif $gimme->topic->defined }}Thema: {{ $gimme->topic->name }}{{ elseif $gimme->publication->identifier == "2" }}{{ if $gimme->article->type_name == "weather_page" }}<a href="/de/static/pages/11177/">Wetter{{ elseif $gimme->section->defined }}<a href="{{ url options="section" }}">{{ $gimme->section->name }}</a>{{ /if }}{{ else }}<a href="{{ url options="issue" }}">Blogs</a>{{ /if }}{{/block}}</h2>
