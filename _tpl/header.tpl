...
{{ dynamic }}
        <div class="content-top">
          ...
          <h2>{{block page_name}}{{ if $gimme->template->name == "front.tpl" }}Das Online-Magazin für Luzern und Zug{{ elseif $gimme->template->name == "search_index.tpl" }}Suchergebnisse{{ elseif $gimme->template->name == "ticker.tpl" }}Ticker{{ elseif $gimme->template->name == "404.tpl" }}Server Error 404{{ elseif $gimme->topic->defined }}Thema: {{ $gimme->topic->name }}{{ elseif $gimme->publication->identifier == "2" }}{{ if $gimme->article->type_name == "weather_page" }}Wetter{{ elseif $gimme->section->defined }}{{ $gimme->section->name }}{{ /if }}{{ else }}Blogs{{ /if }}{{/block}}</h2>
