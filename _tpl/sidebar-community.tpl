...
{{ list_community_feeds length="5" }}                    
        ...
        <li class="comment">Neuer Kommentar zu "<a href="{{ $gimme->community_feed->comment->article->url }}">{{ $gimme->community_feed->comment->article->title }}</a>" / <time>{{ include file="_tpl/relative-date.tpl" date=$gimme->community_feed->comment->submit_date }}</time></li>
        {{ /if }}
