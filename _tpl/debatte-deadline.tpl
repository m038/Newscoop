...
{{ else }}
    <p>Diskussion geschlossen am {{ $deadline->format('j.n.Y') }} um 12:00 Uhr {{ if $gimme->article->comment_count }}<a href="{{ url}}#comments"><span class="comm">{{ $gimme->article->comment_count }}</span></a>{{ /if }}</p> 						      
{{ /if }}   
