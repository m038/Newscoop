            ...
            <ul class="tab-nav clearfix ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all" role="tablist">
              ...
              <div class="tab-content">
                ...
                *}}
                ...
                {{* set_issue number="10" *}}
                <div id="comm-1">
                {{ list_search_results_solr rows=10 q='$query$issue' fq={{ $search_query }} qf="title^5 greybox_title^4 motto^4 infolong^3 teaser^3 pro_title^3 contra_title^3 lede^3 greybox^2 description date_time_text other body pro_text contra_text" start=$smarty.get.start }}
                  ...
                  {{ if $gimme->current_list->at_end }}                   
                    ...
                    <ul class="paging center top-line">
                      {{ if $curpage gt 1 }}
                      <li><a class="button white prev" href="?q={{ $query }}{{ $issue }}&type={{ $getType }}&published={{ $getPublished }}&start={{ $prevstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-1">‹</a></li>
                      {{ /if }}
                      <li class="caption">{{ $curpage }} von {{ ceil($gimme->current_list->count / 10) }}</li>
                      {{ if $gimme->current_list->has_next_elements }}
                      <li><a class="button white next" href="?q={{ $query }}{{ $issue }}&type={{ $getType }}&published={{ $getPublished }}&start={{ $nextstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-1">›</a></li>
                      {{ /if }}
                    </ul>                 
                  {{ /if }} 
                ...
                <div id="comm-2">
                {{ list_search_results_solr rows=10 q='$query$issue' fq="type:news and type:newswire" qf="title^5 greybox_title^4 motto^4 infolong^3 teaser^3 pro_title^3 contra_title^3 lede^3 greybox^2 description date_time_text other body pro_text contra_text" start=$smarty.get.start }}
                  {{ if $gimme->current_list->at_beginning }}
                  <ul>
                  {{ /if }}
                    <li class="news_item  {{ cycle values="odd,even" }}">
                  ...
                  {{ if $gimme->current_list->at_end }}                   
                    ...
                    <ul class="paging center top-line">
                      {{ if $curpage gt 1 }}
                      <li><a class="button white prev" href="?q={{ $query }}{{ $issue }}&type=news+and+newswire&published={{ $getPublished }}&start={{ $prevstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-2">‹</a></li>
                      {{ /if }}
                      <li class="caption">{{ $curpage }} von {{ ceil($gimme->current_list->count / 10) }}</li>
                      {{ if $gimme->current_list->has_next_elements }}
                      <li><a class="button white next" href="?q={{ $query }}{{ $issue }}&type=news+and+newswire&published={{ $getPublished }}&start={{ $nextstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-2">›</a></li>
                      {{ /if }}
                    </ul>                 
                  {{ /if }} 
                ...
                <div id="comm-3">
                {{ list_search_results_solr rows=10 q='$query$issue' fq="type:blog" qf="title^5 greybox_title^4 motto^4 infolong^3 teaser^3 pro_title^3 contra_title^3 lede^3 greybox^2 description date_time_text other body pro_text contra_text" start=$smarty.get.start }}
                  {{ if $gimme->current_list->at_beginning }}
                  <ul>
                  {{ /if }}
                    <li class="news_item  {{ cycle values="odd,even" }}">
                  ...
                  {{ if $gimme->current_list->at_end }}                   
                    ...
                    <ul class="paging center top-line">
                      {{ if $curpage gt 1 }}
                      <li><a class="button white prev" href="?q={{ $query }}{{ $issue }}&type=blog&published={{ $getPublished }}&start={{ $prevstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-3">‹</a></li>
                      {{ /if }}
                      <li class="caption">{{ $curpage }} von {{ ceil($gimme->current_list->count / 10) }}</li>
                      {{ if $gimme->current_list->has_next_elements }}
                      <li><a class="button white next" href="?q={{ $query }}{{ $issue }}&type=blog&published={{ $getPublished }}&start={{ $nextstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-3">›</a></li>
                      {{ /if }}
                    </ul>                 
                  {{ /if }} 
                ...
                <div id="comm-4">
                {{ list_search_results_solr rows=10 q='$query$issue' fq="type:dossier" qf="title^5 greybox_title^4 motto^4 infolong^3 teaser^3 pro_title^3 contra_title^3 lede^3 greybox^2 description date_time_text other body pro_text contra_text" start=$smarty.get.start }}
                  {{ if $gimme->current_list->at_beginning }}
                  <ul>
                  {{ /if }}
                    <li class="news_item  {{ cycle values="odd,even" }}">
                  ...
                  {{ if $gimme->current_list->at_end }}                   
                    ...
                    <ul class="paging center top-line">
                      {{ if $curpage gt 1 }}
                      <li><a class="button white prev" href="?q={{ $query }}{{ $issue }}&type=dossier&published={{ $getPublished }}&start={{ $prevstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-4">‹</a></li>
                      {{ /if }}
                      <li class="caption">{{ $curpage }} von {{ ceil($gimme->current_list->count / 10) }}</li>
                      {{ if $gimme->current_list->has_next_elements }}
                      <li><a class="button white next" href="?q={{ $query }}{{ $issue }}&type=dossier&published={{ $getPublished }}&start={{ $nextstart }}&fqfrom={{ $from }}&fqto={{ $to }}#comm-4">›</a></li>
                      {{ /if }}
                    </ul>                 
                  {{ /if }} 
