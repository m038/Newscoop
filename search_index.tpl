...
{{ /image }}                        
                        ...
                        {{ if ($gimme->article->type_name !== "restaurant") && ($gimme->article->type_name !== "static_page") }}<time>{{ if $gimme->article->type_name == "newswire" }}Von Swiss txt{{ elseif $gimme->article->type_name == "blog" }}Blogbeiträge{{ elseif $gimme->article->type_name == "dossier" }}Dossier{{ else }}Artikel{{ /if }}, {{ include file="_tpl/relative-date.tpl" date=$gimme->article->publish_date }}</time>{{ /if }}
                    </article>
