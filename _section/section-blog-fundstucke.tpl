		...
		{{ list_articles length="5" constraints="type is blog" }}             
                    ...
                    <figure>
                     {{ list_article_images length="2" }}
                    	<a href="{{ url options="article" }}"><img src="{{ uri options="image width 347 height 247 crop center" }}" class="{{ if $gimme->current_list->index == 1 }}left{{ else }}right{{ /if }}" alt="{{ $gimme->article->image->description }}" /></a>
                    	{{ /list_article_images }}
                    </figure>                        
                    {{ elseif $gimme->article->fundstucke_big }}
                    <figure>
                     {{ list_article_images length="1" }}
                    	<a href="{{ url options="article" }}"><img src="{{ uri options="image width 722" }}" alt="{{ $gimme->article->image->description }}" /></a>
                    	{{ /list_article_images }}
                    </figure>
                    {{ elseif $gimme->article->fundstucke_left }}
                    <figure class="left">
                     {{ list_article_images length="1" }}
                    	<a href="{{ url options="article" }}"><img src="{{ uri options="image width 347" }}" alt="{{ $gimme->article->image->description }}" /></a>
                    	{{ /list_article_images }}
                    </figure>
                    ...
                    {{ /if }}
                        ...
                        <p>{{ include file="_tpl/_admin-edit.tpl" }}{{ $gimme->article->lede|strip_tags:false }} <a href="{{ url options="article" }}">Zur Galerie</a> {{ if $gimme->article->comment_count }}<span class="comm">{{ $gimme->article->comment_count }}</span>{{ /if }}</p>
                    </article>
