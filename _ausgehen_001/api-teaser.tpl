...
{{ /local }}
    ...
    if (window.event_teaser && window.event_teaser.events && window.event_teaser.events[0]) {
        ...
        if (use_event && use_event['title'] && use_event['event_key']) {
            ...
            var event_display = "<h6>Tipps des Tages</h6>";
            event_display += "<figure><img src=\"{{ uri static_file="pictures/" }}" + cur_icon_name + "\" alt=\"" + event_icon_alt + "\" /></figure>\n";
            event_display += "<h3><a href=\"" + event_base_dir + "?event_key=" + use_event['event_key'] + "&date=" + event_date + "\">" + use_event['title'] + "</a></h3>\n";

            event_display += "<p>" + event_desc;
            event_display += "<br /><a href=\"" + event_base_dir + "?event_key=" + use_event['event_key'] + "&date=" + event_date + "\">weiterlesen</a></p>";
...
*/
            ...
            var movie_display = "<h6>Neu im Kino</h6>";
            movie_display += "<figure><img src=\"{{ uri static_file="pictures/" }}" + cur_icon_name + "\" alt=\"" + movie_icon_alt + "\" /></figure>\n";
            movie_display += "<h3><a href=\"" + movie_base_dir + "?movie_key=" + use_movie['movie_key'] + "&date=" + cinema_date + "&region=" + cinema_region + "\">" + use_movie['title'] + "</a></h3>";

            movie_display += "<p>" + movie_desc;
            movie_display += "<br /><a href=\"" + movie_base_dir + "?movie_key=" + use_movie['movie_key'] + "&date=" + cinema_date + "&region=" + cinema_region + "\">weiterlesen</a></p>";
