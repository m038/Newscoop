            ...
            <h3>{{ $gimme->article->headline|replace:'\\':'\'' }}</h3>
                ...
                {{ if $gimme->article->has_image(1) }}
                    <figure class="clearfix">
                        ...
                        {{ if 0 && $gimme->article->rest_panorama_count }}
                        ...
                        {{ /if }}
                            <div class="img-options clearfix">
                                ...
                                {{ if $gimme->article->rest_panorama_count }}
                                    <a href="#" onClick='$("#rest_panorama").hide();$("#rest_photo").show();return false;' class="button white right">Fotos</a>
                                    <a href="#" onClick='$("#rest_photo").hide();$("#rest_panorama").show();return false;' class="button white active right">Panorama</a>
                                {{ /if }}
                            </div>
                    </figure>
                {{ /if }}
...
{{ /if }}
{{ if $rest_rating }}
                        <tr><span>Gault-Millau:</th><td>{{ $rest_rating }}/20</td></tr>
{{ /if }}
{{ if $date_time_text }}
                        <tr><span>Geöffnet</th><td>{{ $date_time_text }}</td></tr>
{{ /if }}
{{ if $services_text }}
                        <tr><span>Services:</th><td>{{ $services_text }}</td></tr>
{{ /if }}
{{ if $gimme->article->rest_seats_in || $gimme->article->rest_seats_out }}
                        <tr><span>Sitzplätze:</span>innen: {{ 0 + $gimme->article->rest_seats_in }}, aussen: {{ 0 + $gimme->article->rest_seats_out }}</li>
{{ /if }}
                    </table>

                    <div class="tabs article-tabs">
                        <ul class="tab-nav clearfix">
                            <li><a href="#" class="rest_link_menu_any" id="rest_menu_link" onClick="toggle_rest_menu(); return false;">Tagesmenü</a></li>
                            <li><a href="#inner-tab-1">Tagesmenü</a></li>
                            {{ if $speciality_count }}<li><a href="#" class="rest_link_menu_any" id="rest_speciality_link" onClick="toggle_rest_speciality(); return false;">Hausspezialiäten</a></li>{{ /if }}
