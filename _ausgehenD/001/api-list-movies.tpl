...
window.api_mode = "list";
{{ if $movie_type eq "kino" }}
    window.api_list = {{ api_cinema cinema_date=$cinema_date cinema_region=$cinema_region service_provider="http://edge.sourcefabric.org/newscoop/wobs-motm-13-events-for-luzern" http_userpwd="dev:SoFab" }};
{{ else }}
    window.api_list = {{ api_cinema movie_genre=$movie_type cinema_date=$cinema_date cinema_region=$cinema_region service_provider="http://edge.sourcefabric.org/newscoop/wobs-motm-13-events-for-luzern" http_userpwd="dev:SoFab" }};
{{ /if }}
console.log(window.api_list);
...
function get_cinema(cinema_id) {
    if ((!window.api_list) || (!window.api_list.locations)) {
        return null;
    }
    var cinema_list = window.api_list.locations;
...
$(document).ready(function() {
    ...
    if (window.api_list && window.api_list.films) {
        list_content = '';

        var api_dose = window.api_list.films;
...
*/
            ...
            list_content += "<li style=\"min-height:300px;\">";

            list_content += "<div>";
            if (cur_movie.movie_poster_url) {
                list_content += "<div style=\"float:right;\">";
                list_content += "<img src=\"" + cur_movie.movie_poster_url + "\">";
                list_content += "</div>";
            }

            list_content += "<h3><a href=\"" + cur_url_base + "?date=" + "{{ $cinema_date }}" + "&movie_key=" + cur_movie_key + "&region=" + "{{ $cinema_region }}" + "\">" + cur_movie.title + "</a></h3>";
            list_content += "</div>";
            list_content += "<div>";
            if (cur_movie.synopsis) {
                list_content += cur_movie.synopsis;
            }
            ...
            if (cur_movie.screenings) {
                ...
                for (var css_ind = 0; css_ind < cur_screen_set_count; css_ind++) {
                    ...
                    list_content += "<div>";
                    list_content += "<ul>" + cur_cin_name + "<br />" + cur_cin_addr;
                    ...
                    if (cur_scr_times) {
                        ...
                        for (var cst_ind = 0; cst_ind < cur_scr_times_count; cst_ind++) {
                            ...
                            var cur_scr_datetime = cur_scr_time_info.date_time;

                            list_content += "<li>" + cur_scr_datetime;
                            list_content += "</li>";
