...
{{ if $smarty.get.region }}
    {{ assign var="movie_region" value=$smarty.get.region|replace:" ":"\\ "|replace:'"':"" }}
{{ /if }}
...
{{ if $cinema_region eq "" }}
    ...
    $(document).ready(function() {
        update_subnav_links("{{ $cinema_date }}", 1, "kanton-basel-stadt");
    });
{{ else }}
...
*/
    ...
    if (window.api_detail && window.api_detail.film) {
        var got_movie = window.api_detail.film;
        detail_content = "";
...
*/
        ...
        detail_content += "<div>" + got_movie.title + "</div>";

        detail_content += "<ul>";

        if (got_movie.rating) {
            detail_content += "<li>rating: " + got_movie.rating + "</li>";
        }
        if (got_movie.director) {
            detail_content += "<li>director: " + got_movie.director + "</li>";
        }
        if (got_movie.actors) {
            detail_content += "<li>actors: " + got_movie.actors + "</li>";
        }
